...
import { BULLION_METALS } from "@/lib/bullion";

export default function AddBullionForm({
  categories,
  mode = "position",
}: {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(body.error ?? "Failed to create bullion");
        return;
      }
      formEl.reset();
      setOpen(false);
      ...
      setError("Failed to create bullion");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-border/50 pb-4">
        <h3 className="text-lg font-display font-medium text-foreground">Add New Bullion</h3>
        <button
          type="button"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        <label className="block space-y-1.5">
          <span className="text-xs font-mono uppercase tracking-wider text-muted-foreground">Metal</span>
          <select
            name="symbol"
            required
            defaultValue=""
          ...
          >
            ...
            {BULLION_METALS.map((m) => (
              <option key={m.symbol} value={m.symbol}>
                {m.name} ({m.symbol})
              </option>
            ))}
          </select>
          <span className="text-[10px] text-muted-foreground/70">name and icon are set automatically</span>
        </label>
        <label className="block space-y-1.5">
